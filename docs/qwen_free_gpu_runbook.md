...
From the mounted repository root, run one foreground command:
...
The runner finds the canonical input, downloads the pinned model once, launches one worker per T4, validates and merges exactly two shards, and prints the archive path and SHA-256.
Download `/kaggle/working/qwen3-4b-s009.zip` only after it reports all 100 documents.
